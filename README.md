# OpenAI Smart Diagnosis Lambda

Python AWS Lambda function that uses AWS SAM (Serverless Application Model), consistig on a smart medical assistant that communicates with ChatGPT (a powerful language model) to assist doctors
...
```bash
chatgpt-diagnosis-lambda$ sam build --use-container
```

The SAM CLI installs dependencies defined in `base/requirements.txt`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
chatgpt-diagnosis-lambda$ sam local invoke OpenAIDiagnosisFunction --event events/event-v2.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
chatgpt-diagnosis-lambda$ sam local start-api
chatgpt-diagnosis-lambda$ curl http://localhost:3000/
```
...
```bash
chatgpt-diagnosis-lambda$ sam logs -n OpenAIDiagnosisFunction --stack-name chatgpt-diagnosis-lambda --tail
```
...
```bash
chatgpt-diagnosis-lambda$ pip install -r tests/requirements.txt
# unit test
chatgpt-diagnosis-lambda$ python -m pytest tests/unit -v
```
...
```bash
sam delete --stack-name "chatgpt-diagnosis-lambda"
```

## Powertools for AWS Lambda (Python) features

Although Powertools for AWS Lambda (Python) provides three core utilities (logging, tracing
and metrics), we just used logging:
